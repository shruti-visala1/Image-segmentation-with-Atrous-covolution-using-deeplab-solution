...
app = Flask(__name__)
app._static_folder ='static/opt'
CORS(app)
# STATIC_DIR = os.path.join('static','opt')
...
def vis_segmentation(image, seg_map):
  ...
  plt.grid('off')
  plt.show()
  # plt.savefig( os.path.join(STATIC_DIR , 'img.jpg'))
  plt.close()
...
def run_visualization(img_path ,model_path ):
    # img_path, model_path = 'demo/coco_test_12510.jpg' , 'model/deeplab_model.tar.gz'
    model = DeepLabModel(model_path)
    img = Image.open(img_path)
    resized_im, seg_map = model.run(img)
    vis_segmentation(resized_im, seg_map)

    # return render_template('index.html', name = os.path.join('static/opt','img.jpg'))
...
if __name__=='__main__':
    ...
    model_path = 'deeplab_model.tar.gz'
    run_visualization(img_path ,model_path )
